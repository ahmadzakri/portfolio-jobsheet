...
const JOBSHEET_BUCKET = "jobsheets";
...
async function readCloudLinks(player) {
  try {
    ...
    console.warn("Cloud jobsheet links unavailable:", error);
    return [];
  }
}
...
async function saveCloudLink(player, round, data) {
  const response = await fetch(`${SUPABASE_URL}/rest/v1/jobsheets?on_conflict=player,round`, {
    ...
    body: JSON.stringify({ player, round, ...data, updated_at: new Date().toISOString() }),
  });
  if (!response.ok) throw new Error(await response.text());
  return response.json();
}

async function uploadJobsheetPdf(player, round, file) {
...
async function initCloudPlayerLog() {
  ...
  grid.addEventListener("click", async (event) => {
    const uploadButton = event.target.closest("button[data-upload]");
    if (uploadButton) {
      const index = Number(uploadButton.dataset.upload);
      const card = uploadButton.closest(".jobsheet");
      const file = card.querySelector('input[data-field="pdf-file"]').files[0];
      const status = card.querySelector("[data-status]");
      try {
        uploadButton.disabled = true;
        uploadButton.textContent = "UPLOADING...";
        status.textContent = "Uploading PDF to cloud storage...";
        const result = await uploadJobsheetPdf(player, index + 1, file);
        const live = card.querySelector('input[data-field="live"]').value.trim();
        links[index] = { ...links[index], live, pdf: result.pdfUrl, filePath: result.path };
        localStorage.setItem(linkKey(player), JSON.stringify(links));
        await saveCloudLink(player, index + 1, { live_url: live || null, pdf_url: result.pdfUrl, file_path: result.path });
        status.textContent = "UPLOAD COMPLETE ✓ Everyone can open this PDF.";
        uploadButton.textContent = "UPLOADED ✓";
        uploadButton.classList.add("is-saved");
    ...
    if (saveButton) {
      ...
      const card = saveButton.closest(".jobsheet");
      const live = card.querySelector('input[data-field="live"]').value.trim();
      links[index] = { ...links[index], live };
      localStorage.setItem(linkKey(player), JSON.stringify(links));
      try {
        saveButton.disabled = true;
        saveButton.textContent = "SAVING...";
        await saveCloudLink(player, index + 1, {
          live_url: live || null,
  ...
  render();
  const cloudRows = await readCloudLinks(player);
  cloudRows.forEach((row) => {
    const index = Number(row.round) - 1;
    if (index < 0 || index >= TOTAL) return;
    links[index] = {
      ...links[index],
      live: row.live_url || links[index].live || "",
      pdf: row.pdf_url || links[index].pdf || "",
